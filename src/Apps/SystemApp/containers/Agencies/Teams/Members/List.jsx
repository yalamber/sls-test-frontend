...
class MemberList extends Component {
  ...
  fetchData = async (teamId, page) => {
    try {
      ...
      let userData = await SWQAClient.getAgencyTeamMembers(match.params.teamId, {
  ...
  render() {
    ...
    return (
      <List {...this.props}
        onTableRow={(row) => ({
          onDoubleClick: () => push(`/admin/agency/team/${row.teamId}/member/${row.userId}/details`)
        })}
        loading={this.state.loading}
        title="Members"
        pageHeader={!this.state.loading && [`Agency - ${ get(this.state, 'team.agency.name', '') }`, <br key="0" />, `Team - ${ get(this.state, 'team.name', '') }`]}
        ...
        paginationOptions={{
          ...
          onChange: (page) => push(`/admin/agency/team/${match.params.teamId}/members?page=${page}`)
        }}
        rowKey="userId" />
    );
  }
